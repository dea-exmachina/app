-- [catchup] nexus-users catch-up to product baseline
-- Date: 2026-02-25
-- Applies to: dea-nexus-users (aospwxlomkwpntmbnoee)
-- Source: dea-exmachina-admin (hehldpjqlxhshdqqadng) — post Phase 2
--
-- Starting state (nexus-users has 11 tables):
--   nexus_projects, nexus_cards, nexus_comments, nexus_events, nexus_locks,
--   nexus_task_details, nexus_context_packages, nexus_agent_sessions,
--   nexus_alerts, user_settings, nexus_user_context
--
-- This migration adds:
--   - signal_domain ENUM (nexus_framework_id already exists)
--   - Helper functions: normalize_newlines, log_audit
--   - 25+ trigger functions
--   - 4 platform-seed tables + seed data
--   - 32 missing product tables with RLS
--   - Missing columns on existing tables (nexus_cards, nexus_projects,
--     nexus_task_details, nexus_alerts)
--   - Triggers on existing tables (validation, audit, business logic)
--   - _schema_meta + _migration_log management tables
-- ===========================================================================
...
-- ENUM TYPES
-- ===========================================================================
-- nexus_framework_id already exists in nexus-users; skip.
DO $$ BEGIN
...
-- ===========================================================================
-- TRIGGER FUNCTIONS (CREATE OR REPLACE — safe to run on existing DB)
-- ===========================================================================
...
BEGIN
  IF NEW.lane = 'done' AND OLD.lane != 'done'
    ...
    INSERT INTO bender_performance (bender_slug, bender_name, task_id, level, reviewed_by, user_id)
    VALUES (NEW.assigned_to, NEW.assigned_to, NEW.card_id, 'pending', 'pending', auth.uid())
...
BEGIN
  ...
  PERFORM log_audit('created','card','nexus_cards',NEW.id,NULL,
    ...
    '{}', v_actor,
    CASE WHEN v_actor IN ('system','trigger') THEN 'system' WHEN v_actor LIKE 'bender%' THEN 'bender' ELSE 'dea' END,
...
BEGIN
  ...
  PERFORM log_audit('created','comment','nexus_comments',NEW.id,NULL,
    ...
    '{}', NEW.author,
    CASE WHEN NEW.author IN ('system','trigger') THEN 'system' WHEN NEW.author LIKE 'bender%' THEN 'bender' ELSE 'dea' END,
...
BEGIN
  ...
  v_actor_type := CASE WHEN v_actor LIKE 'bender%' THEN 'bender' WHEN v_actor IN ('system','trigger') THEN 'system' ELSE 'dea' END;
  IF TG_OP = 'INSERT' THEN
    PERFORM log_audit('created','team','bender_teams',NEW.id,NULL,jsonb_build_object('name',NEW.name,'slug',NEW.slug),'{}',v_actor,v_actor_type,NEW.project_id,NULL,'trigger'); RETURN NEW;
  ELSIF TG_OP = 'UPDATE' THEN
...
-- ===========================================================================
-- SEED: nexus_lane_transitions (must exist before validate_lane_transition trigger)
-- ===========================================================================
...
-- ===========================================================================
-- NEW PRODUCT TABLES (in FK-safe order)
-- Existing tables: nexus_projects, nexus_cards, nexus_comments, nexus_events,
--   nexus_locks, nexus_task_details, nexus_context_packages, nexus_agent_sessions,
--   nexus_alerts, user_settings, nexus_user_context
-- ===========================================================================

-- nexus_workstreams — must exist before nexus_cards columns are added
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='nexus_workstreams' AND policyname='nexus_workstreams_select') THEN
...
-- nexus_sprints — must exist before nexus_cards columns are added
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='nexus_sprints' AND policyname='nexus_sprints_select') THEN
...
-- projects (non-nexus, general projects)
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='bender_identities' AND policyname='bender_identities_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='bender_teams' AND policyname='bender_teams_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='bender_team_members' AND policyname='bender_team_members_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='bender_tasks' AND policyname='bender_tasks_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='bender_performance' AND policyname='bender_performance_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='project_benders' AND policyname='project_benders_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='project_tech_stack' AND policyname='project_tech_stack_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='project_workflows' AND policyname='project_workflows_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='identity_project_context' AND policyname='identity_project_context_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='identity_recommendations' AND policyname='identity_recommendations_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='nexus_card_reopens' AND policyname='nexus_card_reopens_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='nexus_routing_rules' AND policyname='nexus_routing_rules_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='nexus_token_usage' AND policyname='nexus_token_usage_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='workspace_scope' AND policyname='workspace_scope_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='workspace_sessions' AND policyname='workspace_sessions_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='workspace_tokens' AND policyname='workspace_tokens_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='workspace_artifacts' AND policyname='workspace_artifacts_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='workspace_audit_events' AND policyname='workspace_audit_events_select') THEN
...
-- wisdom_docs (uses signal_domain ENUM)
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='wisdom_docs' AND policyname='wisdom_docs_select') THEN
...
-- learning_signals (append-only, uses signal_domain)
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='learning_signals' AND policyname='learning_signals_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='signal_context_feedback' AND policyname='signal_context_feedback_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='routing_config' AND policyname='routing_config_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='task_type_routing' AND policyname='task_type_routing_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='user_learnings' AND policyname='user_learnings_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='inbox_items' AND policyname='inbox_items_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='canvases' AND policyname='canvases_select') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='workflows' AND policyname='workflows_select') THEN
...
-- nexus_projects: add group_slug, lane_config
ALTER TABLE nexus_projects ADD COLUMN IF NOT EXISTS group_slug  TEXT;
ALTER TABLE nexus_projects ADD COLUMN IF NOT EXISTS lane_config JSONB;

-- nexus_cards: add 9 missing columns
-- workstream_id + sprint_id reference tables created above
ALTER TABLE nexus_cards ADD COLUMN IF NOT EXISTS lane_changes         INTEGER DEFAULT 0;
ALTER TABLE nexus_cards ADD COLUMN IF NOT EXISTS version              INTEGER NOT NULL DEFAULT 1;
ALTER TABLE nexus_cards ADD COLUMN IF NOT EXISTS ready_for_production BOOLEAN DEFAULT false;
ALTER TABLE nexus_cards ADD COLUMN IF NOT EXISTS delegation_bypassed  BOOLEAN DEFAULT false;
ALTER TABLE nexus_cards ADD COLUMN IF NOT EXISTS bypass_justification TEXT;
ALTER TABLE nexus_cards ADD COLUMN IF NOT EXISTS workstream_id        UUID REFERENCES nexus_workstreams(id);
ALTER TABLE nexus_cards ADD COLUMN IF NOT EXISTS sprint_id            UUID REFERENCES nexus_sprints(id);
ALTER TABLE nexus_cards ADD COLUMN IF NOT EXISTS blocked_by           TEXT[] DEFAULT '{}';
ALTER TABLE nexus_cards ADD COLUMN IF NOT EXISTS reviewed             BOOLEAN NOT NULL DEFAULT false;

-- nexus_task_details: add test_plan, output_path
ALTER TABLE nexus_task_details ADD COLUMN IF NOT EXISTS test_plan   TEXT;
ALTER TABLE nexus_task_details ADD COLUMN IF NOT EXISTS output_path TEXT;

-- nexus_alerts: add metadata
ALTER TABLE nexus_alerts ADD COLUMN IF NOT EXISTS metadata JSONB DEFAULT '{}';

-- ===========================================================================
-- TRIGGERS
-- All use DROP TRIGGER IF EXISTS + CREATE TRIGGER for idempotency
-- ===========================================================================
...
-- ===========================================================================
-- RPC: create_nexus_card (update to use normalize_newlines)
-- ===========================================================================
...
INSERT INTO _schema_meta (key, value) VALUES
  ...
  ('last_migration', '20260225000004_nexus_users_catchup'),
  ('provisioned_at', now()::text),
  ('catchup_applied_at', now()::text)
ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = now();

-- Record this migration
INSERT INTO _migration_log (migration_version, migration_name, applied_by, status)
VALUES ('20260225000004', 'nexus_users_catchup', 'manual', 'success');
