 ...
 *
 * @param event - A QueenEvent of type webhook.received (or similar)
 * @returns InboundSyncResult with the sync state and entity
 */
export async function processInboundEvent(event: QueenEvent): Promise<InboundSyncResult> {
...
async function pushToExternal(
  ...
  console.info(
    `[QUEEN/sync] Push intent: ${source}/${externalId} -> status "${status}" ` +
    `(connector push not yet implemented for "${source}")`
  )
}

/**
 * Emit a typed sync event into the QUEEN pipeline.
 */
async function emitSyncEvent(
  ...
  try {
    ...
    console.error(
      `[QUEEN/sync] Failed to emit ${eventType} event for ${source}/${externalId}:`,
      err
    )
  }
}
