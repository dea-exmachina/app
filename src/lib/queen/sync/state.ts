...
import type { SyncState } from '@/types/queen'
...
export async function listSyncStates(filter?: SyncStateFilter): Promise<SyncState[]> {
  ...
  if (error) {
    console.error('[QUEEN/sync] Error listing sync states:', error)
    return []
  }
...
export async function upsertSyncState(input: SyncStateCreate): Promise<SyncState | null> {
  ...
  if (error) {
    console.error('[QUEEN/sync] Error upserting sync state:', error)
    return null
  }
...
export async function updateSyncState(
  ...
  if (error) {
    console.error('[QUEEN/sync] Error updating sync state:', error)
    return null
  }
