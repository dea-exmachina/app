 ...
 * }
 *
 * @param eventPayload - The QueenEvent.payload from a webhook.received event
...
function runPipeline(
  ...
  payload: Record<string, unknown>,
  config: TransformConfig,
  entry: { connector: { extract: (p: Record<string, unknown>, c: TransformConfig) => import('@/types/queen').ExtractionResult; transform: (e: InternalEntity, c: TransformConfig) => InternalEntity } }
): TransformResult {
  // ── Stage 2: Extract ──
