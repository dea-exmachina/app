...
export interface EventConsumerOptions {
   ...
   */
  idempotencyKey?: (event: QueenEvent) => string
}
...
export interface EventConsumer {
  ...
  handler: (event: QueenEvent) => Promise<void>
  /** Processing options */
 ...
 */
function matchesFilter(event: QueenEvent, filter: EventConsumerFilter): boolean {
...
export async function processEvents(limit: number = 50): Promise<ProcessResult> {
  ...
  for (const rawEvent of events) {
    const event = rawEvent as QueenEvent
    ...
    for (const consumer of matchingConsumers) {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (attempt < maxRetries) {
            console.warn(
              `[QUEEN] Consumer "${consumer.name}" failed on event ${event.id} (attempt ${attempt + 1}/${maxRetries + 1}): ${lastError}`
            )
          }
        }
      }
      ...
      if (!succeeded) {
        allSucceeded = false
        console.error(
          `[QUEEN] Consumer "${consumer.name}" exhausted retries for event ${event.id}: ${lastError}`
        )
...
async function markProcessed(eventId: string): Promise<void> {
  ...
  if (error) {
    console.error(`[QUEEN] Failed to mark event ${eventId} as processed: ${error.message}`)
  }
}
...
async function sendToDLQ(
  ...
  try {
    ...
    console.error(
      `[QUEEN] CRITICAL: Failed to emit DLQ event for ${originalEvent.id}:`,
      dlqError
    )
  }
}
 ...
 */
export async function replayDLQEvent(dlqEventId: string): Promise<QueenEvent | null> {
  ...
  if (dlqError || !dlqEvent) {
    console.error(`[QUEEN] DLQ event ${dlqEventId} not found`)
    return null
  }

  const originalId = (dlqEvent as QueenEvent).payload?.original_event_id as string
  if (!originalId) {
    console.error(`[QUEEN] DLQ event ${dlqEventId} missing original_event_id in payload`)
    return null
  }
  ...
  if (origError || !original) {
    console.error(`[QUEEN] Original event ${originalId} not found for replay`)
    return null
  }

  const origEvent = original as QueenEvent
  ...
  if (replayError) {
    console.error(`[QUEEN] Failed to replay event ${originalId}: ${replayError.message}`)
    return null
  }

  return replayed as QueenEvent
}
