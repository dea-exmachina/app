/**
 * CREEP Event Emitter Service
 *
 * The canonical way to emit events into the CREEP pipeline.
...
import type { CreepEvent, CreepEventCreate } from '@/types/creep'
...
/**
 * Emit a single event into the CREEP pipeline.
 *
 * Flow: validate -> normalize -> insert -> return persisted event
 *
 * @throws EventEmitError on validation failure or database error
 */
export async function emitEvent(input: CreepEventCreate): Promise<CreepEvent> {
 ...
 */
export async function emitBatch(inputs: CreepEventCreate[]): Promise<CreepEvent[]> {
  ...
  const validated: CreepEventCreate[] = []
